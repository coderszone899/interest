...
function Eval_laguerre(p1) {
    var X = eval_1.Eval(defs_1.cadr(p1));
    var N = eval_1.Eval(defs_1.caddr(p1));
    var p2 = eval_1.Eval(defs_1.cadddr(p1));
    var K = p2 === defs_1.symbol(defs_1.NIL) ? defs_1.Constants.zero : p2;
...
function laguerre2(n, p1, p3) {
    ...
    for (let i = 0; i < n; i++) {
        var result = multiply_1.divide(add_1.subtract(multiply_1.multiply(add_1.add(add_1.subtract(bignum_1.integer(2 * i + 1), p1), p3), Y1), multiply_1.multiply(add_1.add(bignum_1.integer(i), p3), Y0)), bignum_1.integer(i + 1));
        Y0 = Y1;
        Y1 = result;
    }
    return Y1;
